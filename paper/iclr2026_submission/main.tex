...
Long-video benchmarks have grown rapidly in scale and duration
\citep{fu2024videomme, zhou2025mlvu, wu2024longvideobench, ataallah2025infinibench},
but they overwhelmingly ask \emph{what happened} in a video rather than \emph{to whom}.
Questions are typically answerable from global plot structure, topic, or a single
well-chosen frame, and several are known to be partially solvable without watching at
all, InfiniBench reports that models score well above chance from metadata such as video
titles \citep{ataallah2025infinibench}, and CG-Bench was motivated by the observation
that long-video multiple choice can be answered by elimination rather than understanding
\citep{chen2025cgbench}. The closest existing formulations stop just short of identity:
LongVideoBench's referring queries point at video \emph{context} rather than at a
persistent entity \citep{wu2024longvideobench}, and CG-Bench grounds \emph{clues} rather
than \emph{referents} \citep{chen2025cgbench}.

PersistQA targets this complementary axis, and does so by reusing existing footage
rather than filming or scraping new video. Its 449 videos are drawn from three public
long-video corpora 148 from CinePile \citep{rawal2024cinepile}, 170 from MovieChat-1k
\citep{song2024moviechat}, and 131 from LVU \citep{wu2021lvu} while all 3{,}667
questions are newly written and adversarially debiased. The reuse is deliberate rather
than incidental: holding the footage fixed isolates the contribution in what is
\emph{asked} rather than in what is \emph{shown}, and it means our difficulty claims can
be compared directly against the questions those corpora ship with.

Every question names a person by visual
description at one moment and asks about a fine-grained attribute of that same person at
a \emph{different} moment, so answering requires carrying an identity across an
intervening shot boundary a median of 53 cuts per video, roughly one every 3.6
seconds and only then perceiving a detail at the far end. Three design choices make
that requirement measurable rather than merely asserted. First, an adversarial committee
of 13 language models answers every question without the video, and anything the
committee beats is removed or repaired; the resulting text-only accuracy is 12.6\%
against a 12.5\% chance rate, so measured performance reflects vision. Second, each
question carries eight options rather than the four or five used above, halving the
chance rate. Third, we annotate \emph{when} the answer is visible for 3{,}245 of 3{,}299
questions (median 4.4\,s within a 166\,s video), which turns ``\emph{did the model see the
evidence}'' into a measured quantity rather than an inference from accuracy.

The result is markedly harder than its scale suggests. The best of 22 evaluated models
reaches 29.5\%; no question is solved by every model and 22.9\% are solved by none,
parameter count barely predicts accuracy ($r=0.05$). Most directly, a model family
reporting 87.5\% on the original MovieChat-1k questions scores below 20\% on PersistQA
questions built from \emph{the same videos} isolating the difficulty in what is asked
...
\toprule
 & \multicolumn{3}{c}{\textbf{Scale}} & \multicolumn{5}{c}{\textbf{Design properties}} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-9}
\textbf{Benchmark} & \textbf{Vid.} & \textbf{QA} & \textbf{Chance}
  & \textbf{Ref.} & \textbf{X-mom.} & \textbf{Blind} & \textbf{Evid.} & \textbf{F.wall} \\
\midrule
\multicolumn{9}{@{}l}{\emph{General long-video understanding}}\\
EgoSchema~\citep{mangalam2023egoschema}      & 5{,}000 & 5{,}000  & 20\%   & \xmark & \xmark & \xmark & \xmark & \xmark \\
Video-MME~\citep{fu2024videomme}             & 900     & 2{,}700  & 25\%   & \xmark & \xmark & \xmark & \xmark & \xmark \\
MLVU~\citep{zhou2025mlvu}                    & 1{,}242 & 2{,}174  & 25\%   & \xmark & \xmark & \xmark & \xmark & \xmark \\
LongVideoBench~\citep{wu2024longvideobench}  & 753     & 1{,}337  & 21\%   & \pmark & \xmark & \xmark & \xmark & \cmark \\
\addlinespace[2pt]
\multicolumn{9}{@{}l}{\emph{Grounded / long-form narrative}}\\
CG-Bench~\citep{chen2025cgbench}             & 1{,}219 & 12{,}129 & 25\%   & \xmark & \xmark & \pmark & \cmark & \xmark \\
InfiniBench~\citep{ataallah2025infinibench}  & \multicolumn{1}{r}{$\sim$1k\,h} & 87.7k & 25\% & \pmark & \pmark & \xmark & \xmark & \xmark \\
\addlinespace[2pt]
\multicolumn{9}{@{}l}{\emph{Human-centric}}\\
HumanVideo-MME~\citep{cai2025humanvideomme}  & \multicolumn{1}{r}{--} & \multicolumn{1}{r}{--} & -- & \pmark & \xmark & \xmark & \xmark & \xmark \\
\midrule
\textbf{PersistQA (ours)}                    & 449 & 3{,}667 & \textbf{12.5\%}
  & \cmark & \cmark & \cmark & \cmark & \cmark \\
\bottomrule
\end{tabular}
\caption{\textbf{PersistQA against the long-video benchmark landscape.}
\textbf{Ref.}: every question names a specific referent by visual description and cannot be
answered without resolving it. \textbf{X-mom.}: the referent is described at one moment and the
queried attribute occurs at a \emph{different} moment, so the identity must survive an intervening
shot change. \textbf{Blind}: questions answerable without video are removed by an adversarial
committee, \emph{and the post-filter text-only accuracy is reported} (ours: 12.6\% against a
12.5\% chance rate). \textbf{Evid.}: per-question temporal annotation of when the answer is
visible (ours: 3{,}245 of 3{,}299 windows, median 4.4\,s). \textbf{F.wall}: a disjoint training
corpus with zero video overlap (ours: 16{,}808 videos). \pmark~denotes a partial or adjacent
property; see \S\ref{sec:related} for what each partial credit means.
PersistQA is deliberately the smallest and shortest-video benchmark in this table: its videos
median 3.2\,min but carry a median of 53 shot cuts, so referent persistence is dense per unit
time rather than diluted across an hour.}
...
Selecting the right frames beats sampling more of them. A training-free probe scores 64 candidate frames against the question with CLIP and keeps the top eight, which adds 1.0 to 2.7 points on three families at a fixed eight-frame budget. Selection compounds with coverage, since selected 16 and 32 frames beat uniform 16 and 32, and the best configuration reaches 31.4\%, shown in Figure~\ref{fig:coverage}. The rendered-clip pipeline that delivers the selected frames costs VideoChat-Flash 5.8 points against its native video path, against 0.8 for InternVL3-14B and 0.2 for Qwen2.5-VL-7B, so for models with native internal frame handling the delivery mechanism matters more than the selection; against a budget-matched uniform-8 baseline rendered through that same path, selection still gains 2.9 points (Section~\ref{sec:method}). An oracle over five frame strategies reaches about 40\%, which bounds how much of the remaining error is a frame-choice problem.
...
\section{Spending the Budget Where the Question Points}
\label{sec:method}

The diagnosis names its own remedy. An eight-frame grid misses the evidence for 53\% of questions
(Section~\ref{sec:p2}) and the memories built to widen coverage retain evidence and background alike
(Section~\ref{sec:p3}); both allocate the budget \emph{before} reading the question. The remedy is to
allocate it after. What that leaves open is the \emph{unit} of allocation, which we treat as the
experiment rather than an implementation detail.

\textbf{Two paths, one signal.} Both score 64 candidate frames against the question with a frozen
CLIP encoder \citep{clip} and spend eight frames on the highest-scoring content, differing
\emph{only} in the retrieval unit. \textsc{Path 1} keeps the top eight frames wherever they fall;
\textsc{Path 2} keeps the two of eight contiguous segments whose best frame scores highest and
samples inside them, becoming a memory bank encoded once and read from stored embeddings at larger
budgets. Sharing encoder, query, pool and budget makes the gap between them a measurement of
granularity alone. Two controls make each falsifiable: \textsc{random} draws eight frames from the
\emph{same} pool, isolating query conditioning from the budget, and \textsc{uniform-8} renders
uniform frames through the identical pipeline, isolating method from rendering. Neither path is
trained, so this is the training-free instance of what \citet{sevila} and \citet{framevoyager} learn.

\begin{table}[t]
\centering
\caption{Both paths beat their controls at a matched eight-frame budget on every backbone tested
($n=3{,}233$, paired exact McNemar \citep{mcnemar1947}, chance $=\chance$). $\Delta_{\text{rand}}$
is \textsc{Path 1} against eight frames drawn at random from the same pool, the contrast isolating
query conditioning. InternVL3-8B's uniform-8 arm was still running at submission.}
\label{tab:method}
\small
\begin{tabular}{@{}lccccr@{}}
\toprule
Backbone & uniform-8 & random-8 & \textsc{Path 2} & \textsc{Path 1} & $\Delta_{\text{rand}}$ \\
\midrule
InternVL3-14B      & 24.62 & 24.47 & 26.94 & \textbf{27.37} & $+2.91^{***}$ \\
InternVL3-8B       & --    & 22.15 & 24.10 & \textbf{25.55} & $+3.40^{***}$ \\
Ovis2.5-9B         & 22.86 & 22.73 & 24.56 & \textbf{25.36} & $+2.63^{***}$ \\
Qwen2.5-VL-7B      & 17.14 & 17.60 & 17.94 & \textbf{19.27} & $+1.67^{*}$ \\
VideoChat-Flash-7B & 20.07 & 20.14 & 20.91 & \textbf{23.01} & $+2.88^{***}$ \\
\bottomrule
\multicolumn{6}{@{}l}{\footnotesize $^{*}p\le0.011$\quad $^{***}p<0.001$}
\end{tabular}
\end{table}

\textbf{Both work; the frame-level one works better.} \textsc{Path 1} beats its random control on all
five backbones and budget-matched uniform-8 on all four where that arm completed ($+2.13$ to
$+2.94$). \textsc{Path 2} is positive everywhere, significant against uniform-8 on two of four, and
replicates at a different operating point: as a memory bank at a matched 8{,}192 tokens it beats
random-segment retrieval by $+3.82$ ($p{=}0.0093$, $n{=}759$), retrieving the evidence-bearing
segment 48.6\% of the time against 25.6\%.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_method_two_paths.pdf}
\caption{Why the two paths behave as they do. (a) Selection raises the rate at which the eight frames
intersect the evidence window of Section~\ref{sec:p2} by $+16.3$ points over its random control.
(b) Yet an exact decomposition assigns only $37.3\%$ of the gain to that hit rate, since accuracy
\emph{given} a hit also rises ($+2.62$, $p{=}0.049$). (c) Hit rate is non-monotone in concentration,
peaking at frame-level selection.}
\label{fig:method}
\end{figure}

\textbf{Granularity has an optimum.} \textsc{Path 2} appears to trade reach for depth, but
\textsc{Path 1} is higher on \emph{both} axes. The modes instead order along concentration alone
(8.00, 5.45, 4.11, 2.00 segments touched) with an interior optimum, and the mechanism is scale: a
segment spans a median 20.7\,s while the evidence occupies far less, so a fixed grid concentrates at
the wrong granularity, whereas top-$k$ scoring concentrates as much as each question warrants.
Conditioning on hit and depth the two become indistinguishable ($-0.21$, $p{=}0.629$), so the gap
runs entirely through where and how densely the budget lands.

\textbf{What this does not show.} Two to three points leaves \bench{} unsolved, and the gain decays
with budget ($+2.14$ at 8 frames, $+0.51$ at 16), so the claim concerns frame \emph{choice} under a
tight budget and bounds a learned in-model selector from below. The memory bank is established
against its random control, not against budget-matched uniform sampling ($+1.32$, no difference
larger than 3.6 points at 80\% power). Every mechanism quantity conditions on oracle windows and is a
diagnostic, never an achievable accuracy.
...
\label{app:extra}
The selection probe scores 64 candidate frames per video against the question text with a CLIP encoder and keeps the eight highest-scoring frames, without any training. Figure~\ref{fig:clip} shows the per-model lift over uniform sampling at a fixed eight-frame budget, split into a question-conditioned variant that scores against the full question and a referent-conditioned variant that scores against the referent phrase only. Both variants raise accuracy on full-fidelity models by 1.0 to 2.7 points, which confirms that the right eight frames beat eight uniform frames. VideoChat-Flash loses accuracy relative to its native video path, but that comparison is not budget-matched; against uniform-8 rendered through the identical clip pipeline it gains 2.9 points (Section~\ref{sec:method}). The penalty is in the delivery of frames, not in their selection.
...
\caption{Lift of CLIP keyframe selection over uniform sampling per model. The apparent loss for VideoChat-Flash is measured against its native video path and is not budget-matched; against uniform-8 rendered through the identical clip pipeline it gains 2.9 points (Section~\ref{sec:method}).}
\label{fig:clip}
\end{figure}
